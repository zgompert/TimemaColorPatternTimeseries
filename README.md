...
|---------|-----|---------|:-:|:-:|:-:|
| cen4119 | VP  | Stripe  | Y | Y | h1 |
| cen4280 | VP  | Green   | Y | Y | h1 |
| cen4120 | R12 | Green   | Y | Y | h1 |
| cen4122 | R23 | Stripe  | Y | Y |  |
| 24_0016 | VP  | Green   | Y | Y | Y |
| 24_0028 | VP  | Green   | Y | Y |  |
| 24_0029 | VP  | Green   | Y | Y |  |
| 24_0030 | VP  | Green   | Y | Y |  |
| 24_0038 | VP  | Melanic | Y | Y | h1 |
| 24_0039 | VP  | Melanic | Y | Y | Y |
| 24_0072 | R12 | Stripe  | Y | Y | Y |
| 24_0073 | R12 | Green   | Y | Y | Y |
| 24_0087 | VP  | Stripe  | Y | Y | Y |
| 24_0089 | VP  | Stripe  | Y | Y |  |
| 24_0175 | FH  | Stripe  | Y | Y | Y |
| 24_0176 | FH  | Stripe  | Y | Y | h1 |
...
I used the dot plots to score each ahplotype for 6 large SV (see the images below and [TcrSvCalls.csv](TcrSvCalls.csv). I treated VP_G_GUS_2 as the reference for this, but then also used R_S_GS_2 for the inverted translocation on Refugio. Of the six SVs four are reasonably variable. My SV5 (between mountain inversion) distinguishes G and S Refugio from everything else (R_S_072_2 is the exception but matches the main mountain melanic haplotype and thus this individual is a heterozygote). SV 6, the Refugio inverted translocation, distinguishes G and S on refugio, with the S bugs having the inverted translocation).  SV 3, the main mountain inverted translocation is perfectly associated with color in the sense that all individuals with two copies of this allels are melanic and all other are not. SV 2, an inverted translocation on the end of the chromosome has a marginal association with pattern on Hwy154 (see [SVRsummary.R](SVRsummary.R)). Thus, the only thing not yet fully explained is green versus stripe on Hwy154; this maps (in the past) to SV 3 (the main mountain inverted translocation) but appears to be casued by something segregating withing that region, not the inverted translocation itself. I want to dig in on zoomed SV within that region and more GWA probably to nail this dwn.
...
Next, I filtered for coverage, missing, SNP quality and bias scores (see [vcfFilter.pl](vcfFilter.pl)), the results in 171,366 (0029h1), 172,336 (0038h1), 178,753 (0176h1) and 173,706 (4119h2) SNPs, all still in the scratch directory for now.
...
my $miss = 120; # maximum number of individuals with no data
##### this set is for GBS
my $d;
...
while (<IN>){
        ...
        elsif (m/^hap/){ ## this is a sequence line, you migh need to edit this reg. expr.
